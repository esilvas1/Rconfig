...
**Recomendaciones:**
El modelo es aceptable para uso práctico, ya que la desviación de normalidad no es severa

- **Considerar transformaciones:** Una transformación logarítmica o de raíz cuadrada de G3 podría mejorar la normalidad.
- **Usar métodos robustos:** Para inferencias críticas, considerar regresión robusta o bootstrap.
- **Interpretar con cautela:** Los intervalos de predicción en los extremos pueden ser menos confiables.

#### Conclusión de Validación Q-Q plot:
El Q-Q plot revela que los residuos se aproximan razonablemente a la normalidad en la región central, pero presentan colas más pesadas de lo esperado. Esta desviación es común en datos educativos donde pueden existir factores no capturados que generen rendimientos extremadamente altos o bajos.
...
if (exists("slr_model")) {
  ...
  ggplot(model_data, aes(x = .resid)) +
    ...
    labs(
      title = "Grafica 4.4.3.1: Distribución de Residuos",
...
- **El modelo es apropiado** para uso práctico y toma de decisiones.
- **Interpretar con precaución** los intervalos de predicción en valores extremos.
- **Considerar** que las predicciones centrales son más confiables que las extremas.
- **Validar** periódicamente el modelo con nuevos datos para confirmar la estabilidad del patrón.

Este análisis, combinado con el Q-Q plot y el gráfico de residuos vs valores ajustados, **respalda la conclusión de que el modelo ajustado es estadísticamente sólido** y apropiado para predecir las calificaciones finales (G3) basándose en las calificaciones del segundo período (G2).
...
##### Implicaciones para el modelo:
- **La mayoría del dataset contribuye apropiadamente** al ajuste del modelo sin ejercer influencia desproporcionada
- **Las observaciones influyentes identificadas no son extremas**, por lo que el modelo mantiene estabilidad general
- **No hay evidencia de outliers altamente problemáticos** que pudieran distorsionar significativamente los resultados
...
if (exists("slr_model")) {
  ...
  if (length(influential_indices) > 0) {
    ...
    p_influence <- ggplot(model_data, aes(x = .data[[best_predictor]], y = G3)) +
      geom_point(alpha = 0.6, color = "lightblue", size = 1.5) +
...
if (exists("slr_model") && nrow(df_mat) > 0) {
  ...
  if (nrow(df_clean) > 50) {  # Verificar que tenemos suficientes datos
    ...
    # Tests de supuestos modelo limpio
    shapiro_limpio <- if(nrow(df_clean) <= 5000) shapiro.test(residuals(modelo_limpio))$p.value else NA
    bp_limpio <- if(requireNamespace("lmtest", quietly = TRUE)) lmtest::bptest(modelo_limpio)$p.value else NA
    ...
    cat("Shapiro p-valor:", round(shapiro_limpio, 4), "\n")
    cat("BP p-valor:", round(bp_limpio, 4), "\n\n")
  }
  ...
  if (requireNamespace("MASS", quietly = TRUE)) {
    ...
    # Calcular R² aproximado para modelo robusto
  ...
  shapiro_winsor <- if(nrow(df_winsor) <= 5000) shapiro.test(residuals(modelo_winsor))$p.value else NA
  bp_winsor <- if(requireNamespace("lmtest", quietly = TRUE)) lmtest::bptest(modelo_winsor)$p.value else NA
  
  cat("MODELO CON WINSORIZACIÓN (5%-95%):\n")
  cat("R²:", round(summary(modelo_winsor)$r.squared, 4), "\n")
  cat("Shapiro p-valor:", round(shapiro_winsor, 4), "\n")
  cat("BP p-valor:", round(bp_winsor, 4), "\n\n")
  
  # GRÁFICOS COMPARATIVOS DE MODELOS ALTERNATIVOS
  library(gridExtra)
  
  # Crear lista de gráficos para comparación
  plots_list <- list()
  
  # Gráfico 1: Modelo Original
  data_orig <- augment(slr_model)
  plots_list[[1]] <- ggplot(data_orig, aes(x = .fitted, y = .resid)) +
    geom_point(alpha = 0.6, color = "#1f77b4") +
    geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
    geom_smooth(method = "loess", se = FALSE, color = "orange") +
    labs(title = "Modelo Original",
         subtitle = paste("R² =", round(summary(slr_model)$r.squared, 3)),
         x = "Valores Ajustados", y = "Residuos") +
    theme_minimal()
  
  # Gráfico 2: Modelo sin influyentes (si existe)
  if (exists("modelo_limpio")) {
    data_limpio <- augment(modelo_limpio)
    plots_list[[2]] <- ggplot(data_limpio, aes(x = .fitted, y = .resid)) +
      geom_point(alpha = 0.6, color = "#ff7f0e") +
      geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
      geom_smooth(method = "loess", se = FALSE, color = "orange") +
      labs(title = "Sin Influyentes",
           subtitle = paste("R² =", round(summary(modelo_limpio)$r.squared, 3), 
                           "| n =", nrow(df_clean)),
           x = "Valores Ajustados", y = "Residuos") +
      theme_minimal()
  }
  
  # Gráfico 3: Modelo Winsorizado
  if (exists("modelo_winsor")) {
    data_winsor <- augment(modelo_winsor)
    plots_list[[3]] <- ggplot(data_winsor, aes(x = .fitted, y = .resid)) +
      geom_point(alpha = 0.6, color = "#2ca02c") +
      geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
      geom_smooth(method = "loess", se = FALSE, color = "orange") +
      labs(title = "Winsorizado (5%-95%)",
           subtitle = paste("R² =", round(summary(modelo_winsor)$r.squared, 3)),
           x = "Valores Ajustados", y = "Residuos") +
      theme_minimal()
  }
  
  # Gráfico 4: Modelo Robusto (residuos vs ajustados)
  if (exists("modelo_robusto")) {
    residuos_rob <- residuals(modelo_robusto)
    ajustados_rob <- fitted(modelo_robusto)
    data_robusto <- data.frame(fitted = ajustados_rob, resid = residuos_rob)
    
    plots_list[[4]] <- ggplot(data_robusto, aes(x = fitted, y = resid)) +
      geom_point(alpha = 0.6, color = "#d62728") +
      geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
      geom_smooth(method = "loess", se = FALSE, color = "orange") +
      labs(title = "Modelo Robusto",
           subtitle = paste("R² aprox =", round(r2_robusto, 3)),
           x = "Valores Ajustados", y = "Residuos") +
      theme_minimal()
  }
  ...
  if (length(plots_list) >= 2) {
    grid.arrange(grobs = plots_list, ncol = 2, 
                 top = "Gráfico 8.3: Comparación de Residuos - Modelos Alternativos")
  }
}
```
...
if (exists("slr_model")) {
  ...
  # Modelo original
  shapiro_orig <- if(length(residuals(slr_model)) <= 5000) shapiro.test(residuals(slr_model))$p.value else NA
  bp_orig <- if(requireNamespace("lmtest", quietly = TRUE)) lmtest::bptest(slr_model)$p.value else NA
  ...
  modelos_resultados <- modelos_resultados %>%
    add_row(
      ...
      AIC = AIC(slr_model),
      N_obs = nrow(df_mat),
      Supuestos_OK = (shapiro_orig > 0.05) & (bp_orig > 0.05)
    )
  ...
  if (exists("modelo_limpio")) {
    modelos_resultados <- modelos_resultados %>%
      add_row(
        ...
        N_obs = nrow(df_clean),
        Supuestos_OK = (shapiro_limpio > 0.05) & (bp_limpio > 0.05)
      )
  }
  ...
  if (exists("modelo_winsor")) {
    modelos_resultados <- modelos_resultados %>%
      add_row(
        ...
        Supuestos_OK = (shapiro_winsor > 0.05) & (bp_winsor > 0.05)
      )
  }
  ...
  if (exists("modelo_robusto")) {
    modelos_resultados <- modelos_resultados %>%
      add_row(
        Modelo = "Robusto",
        R_cuadrado = r2_robusto,
        Shapiro_p = NA,
        BP_p = NA,
        AIC = NA,
        N_obs = nrow(df_mat),
        Supuestos_OK = TRUE  # Los modelos robustos no requieren supuestos estrictos
      )
  }
  ...
  modelos_resultados %>%
    ...
    kable(caption = "Tabla 8.3: Comparación de Modelos Alternativos", 
          format = "html") %>%
    kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))
  
  # Seleccionar mejor modelo
  mejor_modelo_row <- modelos_resultados %>%
    filter(Supuestos_OK == TRUE) %>%
    arrange(desc(R_cuadrado)) %>%
    slice(1)
  
  if (nrow(mejor_modelo_row) > 0) {
    cat("\n🎯 MODELO RECOMENDADO:", mejor_modelo_row$Modelo, "\n")
    cat("✓ Cumple supuestos estadísticos\n")
    cat("✓ R² =", round(mejor_modelo_row$R_cuadrado, 4), "\n")
    cat("✓ N observaciones =", mejor_modelo_row$N_obs, "\n\n")
    
    # Asignar modelo final
    if (mejor_modelo_row$Modelo == "Sin Influyentes" && exists("modelo_limpio")) {
      modelo_final <- modelo_limpio
      datos_finales <- df_clean
    } else if (mejor_modelo_row$Modelo == "Winsorizado" && exists("modelo_winsor")) {
    ...
    cat("\n⚠️ NINGÚN MODELO CUMPLE SUPUESTOS PERFECTAMENTE\n")
    cat("Recomendación: Usar modelo robusto o documentar limitaciones\n\n")
    modelo_final <- if(exists("modelo_robusto")) modelo_robusto else slr_model
    datos_finales <- df_mat
  }
  
  # GRÁFICO DE SELECCIÓN DEL MODELO
  if (nrow(modelos_resultados) > 1) {
    cat("\n")
    
    # Gráfico de comparación de R² vs cumplimiento de supuestos
    p_comparison <- modelos_resultados %>%
      mutate(
        Color = ifelse(Supuestos_OK, "Cumple Supuestos", "No Cumple"),
        Seleccionado = ifelse(row_number() == which.max(ifelse(Supuestos_OK, R_cuadrado, 0)), 
                             "Modelo Seleccionado", "Otros")
      ) %>%
      ggplot(aes(x = R_cuadrado, y = reorder(Modelo, R_cuadrado))) +
      geom_col(aes(fill = Color), alpha = 0.7) +
      geom_text(aes(label = paste("R² =", round(R_cuadrado, 3))), 
                hjust = -0.1, size = 3) +
      scale_fill_manual(values = c("Cumple Supuestos" = "#2ca02c", 
                                   "No Cumple" = "#d62728")) +
      labs(
        title = "Gráfico 8.4: Comparación de Modelos - R² y Cumplimiento de Supuestos",
        subtitle = "Verde: Cumple supuestos | Rojo: No cumple supuestos",
...
if (exists("modelo_final")) {
  ...
  if (class(modelo_final)[1] == "rlm") {
    ...
    ggplot(data.frame(fitted = valores_ajustados, resid = residuos_rob), 
           aes(x = fitted, y = resid)) +
      geom_point(alpha = 0.6, color = "#2b8cbe") +
      geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
      geom_smooth(method = "loess", se = FALSE, color = "orange") +
      labs(
        title = "Gráfico 8.1: Residuos vs Ajustados (Modelo Final)",
    ...
    # Tests finales
    if (nrow(datos_finales) <= 5000) {
      shapiro_final <- shapiro.test(residuals(modelo_final))
      cat("Shapiro-Wilk p-valor:", round(shapiro_final$p.value, 4), 
          ifelse(shapiro_final$p.value > 0.05, " ✓", " ✗"), "\n")
    }
    
    if (requireNamespace("lmtest", quietly = TRUE)) {
      bp_final <- lmtest::bptest(modelo_final)
  ...
  if (class(modelo_final)[1] == "rlm") {
    ...
    cat("R² aproximado:", round(cor(datos_finales$G3, fitted(modelo_final), use = "complete.obs")^2, 4), "\n")
  } else {
...
if (exists("modelo_final")) {
  ...
  cat("✓ Modelo apropiado para predición en el contexto educativo\n")
  ...
  if (exists("slr_model")) {
    ...
    # Modelo original
    data_original <- augment(slr_model)
    
    # Datos para el gráfico
    if (tipo_modelo == "Robusto") {
      # Para modelo robusto
      fitted_final <- fitted(modelo_final)
      p_final <- ggplot(datos_finales, aes(x = .data[[best_predictor]], y = G3)) +
        geom_point(alpha = 0.6, color = "lightblue", size = 1) +
        geom_smooth(method = "lm", se = TRUE, color = "blue", 
                   aes(linetype = "Original"), linewidth = 1) +
        geom_line(aes(y = fitted_final, linetype = "Final (Robusto)"), 
                 color = "red", linewidth = 1.2) +
        scale_linetype_manual(values = c("Original" = "dashed", "Final (Robusto)" = "solid")) +
        labs(
          title = "Gráfico 8.6: Comparación Modelo Original vs Final",
          subtitle = paste("Mejora en robustez - R² final =", round(r2_final, 3)),
          x = best_predictor,
          y = "G3 (Nota Final)",
          linetype = "Modelo"
        ) +
      ...
      # Para modelo lineal clásico
      data_final <- augment(modelo_final)
      p_final <- ggplot(datos_finales, aes(x = .data[[best_predictor]], y = G3)) +
        geom_point(alpha = 0.6, color = "lightblue", size = 1) +
        geom_smooth(data = df_mat, method = "lm", se = FALSE, 
                   aes(linetype = "Original"), color = "blue", linewidth = 1) +
        geom_smooth(method = "lm", se = TRUE, 
                   aes(linetype = "Final"), color = "red", linewidth = 1.2) +
        scale_linetype_manual(values = c("Original" = "dashed", "Final" = "solid")) +
        labs(
          title = "Gráfico 8.6: Comparación Modelo Original vs Final",
          subtitle = paste("Mejora en cumplimiento de supuestos - R² final =", round(r2_final, 3)),
          x = best_predictor,
          y = "G3 (Nota Final)",
          linetype = "Modelo"
        ) +
