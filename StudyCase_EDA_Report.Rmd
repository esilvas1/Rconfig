...
required_packages <- c("tidyverse", "knitr", "kableExtra", "broom", "ggplot2","pandoc", 
                       "forcats", "scales", "dplyr", "gridExtra", "lmtest", "forecast","httpgd","rmarkdown")
...
if (nrow(df_mat)>0) {
  ...
  nums <- df_mat %>% dplyr::select(where(is.numeric))
  if (ncol(nums) > 0) {
...
if (nrow(df_mat)>0) {
  ...
  if ("G3" %in% names(nums) && ncol(nums) >= 2) {
    ...
    if (length(top_vars) > 0) {
      scatter_df <- df_mat %>% dplyr::select(all_of(c("G3", top_vars))) %>% pivot_longer(-G3, names_to = "variable", values_to = "value")
...
cat_vars <- df_mat %>% dplyr::select(where(~ is.character(.) || is.factor(.)))
...
if (nrow(df_mat) > 0 && ncol(cat_vars) > 0) {
  ...
  if (nrow(low_counts) > 0) {
    ...
    low_counts %>%
      dplyr::select(variable, value2, n, pct) %>%
      distinct() %>%
...
if (nrow(df_por)>0) {
  library(ggplot2)
  # Boxplots por variables numéricas
  nums <- df_por %>% dplyr::select(where(is.numeric))
  if (ncol(nums) > 0) {
    nums_long <- nums %>% pivot_longer(everything(), names_to = "variable", values_to = "value")
    p2 <- ggplot(nums_long, aes(x = variable, y = value)) +
      geom_boxplot(outlier.size = 1, fill = "#74a9cf") +
...
if (nrow(df_por)>0) {
  ...
  if ("G3" %in% names(nums) && ncol(nums) >= 2) {
    ...
    top_vars <- head(cor_with_G3$variable, 6)
    if (length(top_vars) > 0) {
      scatter_df <- df_por %>% dplyr::select(all_of(c("G3", top_vars))) %>% pivot_longer(-G3, names_to = "variable", values_to = "value")
      p4 <- ggplot(scatter_df, aes(x = value, y = G3)) +
        geom_point(alpha = 0.6, size = 1.5, color = "#2b8cbe") +
        geom_smooth(method = "lm", se = FALSE, color = "red") +
        facet_wrap(~variable, scales = "free_x") +
...
cat_vars <- df_por %>% dplyr::select(where(~ is.character(.) || is.factor(.)))
...
if (nrow(df_mat) > 0 && exists("best_predictor")) {
  ...
  stats_table <- df_mat %>%
    dplyr::select(all_of(c(best_predictor, "G3"))) %>%
    summarise(
...
if (exists("slr_model")) {
  ...
  if (length(influential_indices) > 0) {
    ...
    tabla_influyentes <- model_data[influential_indices, ] %>%
      dplyr::select(all_of(c(best_predictor, "G3")), .cooksd) %>%
      mutate(
...
## Validación final del modelos seleccionado
...
# Aplicación: Regresión Lineal Simple (student_por)

En nuestro análisis del dataset `student_por`, aplicamos regresión lineal simple para modelar la relación entre las calificaciones previas (G1 o G2) y la nota final (G3), permitiendo predecir el rendimiento académico final basado en el desempeño previo del estudiante.
...
```{r slr-variable-selection_por, results='asis'}
# Selección de variables predictoras basada en correlación con G3
if (nrow(df_por) > 0) {
  library(dplyr)
  library(knitr) 
  ...
  # Calcular correlaciones con G3 para todas las variables numéricas
  # Usar dplyr:: para evitar conflictos de namespace
  nums <- df_por %>% dplyr::select(where(is.numeric))
  ...
  if ("G3" %in% names(nums)) {
    ...
    # Identificar el mejor predictor
    best_predictor_por <- cor_table$variable[1]
    best_correlation <- cor_table$correlacion[1]

    cat("\n Por lo tanto, la variable seleccionada para la regresión es:", best_predictor_por, 
        "\n con una correlación de:", round(best_correlation, 4), "\n con respecto a G3")
  }
} else {
  cat("Dataset student_mat no disponible para análisis.")
...
```{r slr-scatter-plot_por, fig.width=8, fig.height=6, results='asis'}
# Gráfico de dispersión con línea de tendencia
if (nrow(df_por) > 0 && exists("best_predictor")) {
  library(ggplot2)
  
  # Crear el gráfico de dispersión
  p_scatter <- ggplot(df_por, aes_string(x = best_predictor_por, y = "G3")) +
    geom_point(alpha = 0.6, size = 2, color = "#2b8cbe") +
    geom_smooth(method = "lm", se = TRUE, color = "red", fill = "pink", alpha = 0.3) +
    labs(
      title = paste("Gráfico 7.1: Relación entre", best_predictor_por, "y G3"),
      subtitle = paste("Correlación r =", round(best_correlation, 4)),
      x = paste(best_predictor_por, "(Variable Predictora)"),
      y = "G3 (Nota Final)"
    ) +
    theme_minimal() +
    theme(
  ...
  # Estadísticas descriptivas de ambas variables
  stats_table <- df_por %>%
    dplyr::select(all_of(c(best_predictor_por, "G3"))) %>%
    summarise(
      across(everything(), list(
        n = ~sum(!is.na(.)),
        media = ~round(mean(., na.rm = TRUE), 2),
...
```{r slr-model-fit-por, results='asis'}
# Ajuste del modelo de regresión lineal simple
if (nrow(df_por) > 0 && exists("best_predictor_por")) {
  library(broom)
  
  # Crear fórmula del modelo
  formula_str <- paste("G3 ~", best_predictor_por)
  model_formula <- as.formula(formula_str)
  
  # Ajustar el modelo
  slr_model <- lm(model_formula, data = df_por)
  ...
  cat("\n**Ecuación del modelo ajustado:**")
  cat("\nG3 =", intercept, "+", slope, "×", best_predictor_por)
  cat("\n\n**Interpretación:**")
  cat("\n\n- Por cada unidad adicional en", best_predictor_por, ", G3 aumenta en promedio", slope, "puntos")
  cat("\n\n- El modelo explica", round(model_summary$r.squared * 100, 2), "% de la variabilidad en G3")
...
- **Distribución centrada en cero:** Los residuos se distribuyen alrededor de la línea horizontal en y = 0 (línea roja punteada), lo que indica que el modelo no presenta sesgo sistemático en sus predicciones.
- **Patrón relativamente aleatorio:** No se observa una estructura clara o patrón curvilíneo pronunciado en los residuos, sugiriendo que el supuesto de linealidad se cumple razonablemente bien.
- **Homocedasticidad apropiada:** La dispersión de los residuos se mantiene relativamente constante a lo largo del rango de valores ajustados, indicando que la varianza de los errores es estable (homocedasticidad).

#### Observaciones que Requieren Atención:

- **Curvatura sutil en la línea LOESS:** La línea naranja **LOcally wEighted Scatterplot Smoothing** (suavizado LOESS) muestra una ligera curvatura descendente hacia el extremo derecho, sugiriendo una posible no-linealidad menor en valores altos de la predicción.
- **Presencia de valores extremos:** Se observan algunos residuos con valores absolutos superiores a 5 y hasta cerca de -10, indicando la presencia de observaciones con residuos grandes que podrían ser influyentes.
- **Distribución equilibrada:** Los puntos se distribuyen de manera relativamente uniforme a lo largo del rango de valores ajustados (0-20), lo que es favorable para la estabilidad del modelo.
- **Interpretación para el Modelo G3 ~ G2:** El gráfico sugiere que el modelo de regresión lineal simple es apropiado para estos datos del dataset student_por, con las siguientes consideraciones:

El modelo captura adecuadamente la relación principal entre G2 y G3
Los supuestos de linealidad y homocedasticidad se cumplen satisfactoriamente
La dispersión de residuos es más controlada comparada con el dataset student_mat
La ligera curvatura observada es mínima y no compromete significativamente la validez del modelo

#### Recomendación:
El modelo actual es válido y robusto para predicción, pero se recomienda:

- Identificar y examinar las observaciones con residuos extremos

- Considerar la inclusión de variables predictoras adicionales si están disponibles

- Evaluar si transformaciones menores de las variables mejoran el ajuste

En conjunto, este análisis respalda la solidez del modelo ajustado y su capacidad para hacer predicciones confiables de las calificaciones finales (G3) basadas en las calificaciones del segundo período (G2) para el dataset de portugués.


### Calcular Q-Q plot para normalidad de residuos
  
```{r slr-assumptions2-por}
# Validación de supuestos del modelo
if (exists("slr_model")) {
  library(ggplot2)
  library(gridExtra)
...
La mayoría de los puntos (aproximadamente 85-90%) se alinean bien con la línea roja diagonal, indicando que los residuos se aproximan razonablemente a una distribución normal.
...
- **Colas pesadas en extremos inferiores:** En el extremo izquierdo (cuantiles teóricos < -2), los puntos se desvían por debajo de la línea roja, formando una curva que indica residuos más extremos de lo esperado en la cola inferior.
- **Desviación en cola superior:** En el extremo derecho (cuantiles teóricos > +1), algunos puntos se alejan por encima de la línea, mostrando una desviación moderada hacia valores más altos.
- **Patrón escalonado distintivo:** La forma muestra líneas horizontales agrupadas, característico de variables discretas (calificaciones enteras), donde múltiples estudiantes obtuvieron la misma nota.

#### Interpretación Estadística:

**Implicaciones para el modelo:**
Los residuos muestran aproximación razonable a la normalidad en la región central, pero con desviaciones en los extremos
La estructura en líneas horizontales es esperada para datos de calificaciones discretas y no constituye una violación preocupante
Las desviaciones en las colas son menos pronunciadas que en el dataset student_mat

Impacto en las inferencias:

- **Intervalos de confianza y pruebas t:** Mantienen validez apropiada para la mayoría de predicciones centrales.
- **Validez del modelo:** El modelo es robusto y apropiado para el contexto educativo del dataset student_por
- **Robustez:** Las inferencias son confiables dado el tamaño de muestra y la naturaleza discreta de los datos

**Recomendaciones:**
El modelo es válido y apropiado para uso práctico, con las siguientes consideraciones:

- Reconocer que las líneas horizontales son normales para calificaciones discretas

- Interpretar con cautela las predicciones en los extremos del rango

- El modelo es más estable que el equivalente para student_mat

En conjunto, este análisis confirma la solidez del modelo ajustado y su capacidad para hacer predicciones confiables de las calificaciones finales (G3) basadas en las calificaciones del segundo período (G2) para el dataset de portugués.


### Histograma de Residuos

```{r slr-assumptions3-por}
if (exists("slr_model")) {
  library(ggplot2) 
  # 3. Histograma de residuos
  ggplot(model_data, aes(x = .resid)) +
...
- **Forma bimodal distintiva**: Se observan dos picos claramente definidos en la distribución, indicando una estructura más compleja que sugiere posibles subgrupos en los datos o patrones discretos en las calificaciones.

- **Concentración central muy alta**: La mayor densidad de residuos se encuentra concentrada en el rango [-1, +1], con picos que alcanzan densidades superiores a 0.75, indicando una concentración extrema cerca de cero.

#### Desviaciones Observadas:

- **Diferencia marcada entre curvas**: La **línea roja sólida** (densidad kernel estimada de los residuos) muestra múltiples picos que contrastan dramáticamente con la **línea naranja punteada** (distribución normal teórica), especialmente en la región central.

- **Picos extremadamente pronunciados**: La distribución empírica (roja) muestra picos mucho más altos y estrechos que la distribución normal teórica, sugiriendo una **concentración extrema de valores específicos** característica de datos discretos.

- **Colas prácticamente ausentes**: La distribución empírica muestra densidad casi nula en los extremos, contrastando significativamente con las colas de la distribución normal teórica.

- **Patrón escalonado marcado**: La estructura claramente escalonada refleja la naturaleza discreta de las calificaciones (valores enteros), donde múltiples estudiantes obtienen calificaciones idénticas.

#### Comparación de distribuciones:
- **Histograma (barras azules)**: Muestra la distribución empírica real de los residuos con estructura claramente discreta
- **Curva roja sólida**: Estimación suavizada (kernel) que revela los múltiples picos característicos de datos categóricos
- **Curva naranja punteada**: Distribución normal teórica que no se ajusta a la realidad de los datos discretos

#### Implicaciones para el modelo:
- **Los residuos NO siguen una distribución normal clásica** debido a la naturaleza discreta de las calificaciones
- **Existe leptocurtosis extrema** (concentración central muy alta), típica de variables discretas en contextos educativos
- **La estructura bimodal es característica** de datos de calificaciones donde ciertos valores son más comunes

#### Fortalezas del modelo:
- La distribución centrada en cero confirma que el modelo no tiene sesgo sistemático
- Los picos simétricos sugieren que el modelo captura apropiadamente los patrones centrales
- La concentración extrema indica predicciones muy precisas para la mayoría de casos

#### Limitaciones identificadas:
- La naturaleza discreta de los datos viola el supuesto de normalidad continua
- Los intervalos de confianza basados en normalidad pueden ser inapropiados
- La estructura bimodal sugiere que un enfoque robusto sería más apropiado

#### Conclusión de histograma de residuos:

El histograma de residuos **revela la naturaleza discreta** de los datos educativos del dataset student_por. Aunque la estructura no sigue una distribución normal clásica, **es completamente esperada y apropiada** para este tipo de datos de calificaciones, validando la necesidad de métodos robustos para el análisis.


### Gráfico de distancia de Cook
```{r slr-assumptions4-por}
if (exists("slr_model")) {
  library(ggplot2) 
  # 4. Gráfico de distancia de Cook
  ggplot(model_data, aes(x = seq_along(.cooksd), y = .cooksd)) +
...
- **Línea de referencia clara**: La línea roja punteada horizontal (umbral = 4/n ≈ 0.006) proporciona un criterio objetivo para identificar observaciones potencialmente influyentes.

#### Observaciones que Requieren Atención:

- **Presencia de observaciones por encima del umbral**: Se observan varios puntos que superan la línea de referencia roja (4/n), con valores de distancia de Cook entre 0.01 y aproximadamente 0.09, indicando **observaciones moderadamente influyentes**.

- **Concentración de observaciones influyentes**: Las observaciones con mayor distancia de Cook se distribuyen principalmente en la segunda mitad del dataset (índices desde ~150 hasta ~649), sugiriendo cierta concentración en una región específica de los datos.

- **Valores máximos moderados**: Aunque hay observaciones influyentes, ninguna presenta valores extremadamente altos (>0.1), lo que sería una señal de alarma mayor, manteniéndose por debajo de 0.09.

#### Criterio de evaluación:
- **Umbral conservador**: 4/n ≈ 0.006 (línea roja punteada)
- **Umbral moderado**: 1 (no se alcanza en este gráfico)
- **Observaciones influyentes identificadas**: Aproximadamente 10-15 observaciones superan el umbral conservador
...
El gráfico de distancia de Cook **confirma que el modelo es generalmente robusto** y no está dominado por observaciones atípicas extremas. Aunque se identifican algunas observaciones moderadamente influyentes en el dataset student_por, estas **no comprometen la validez fundamental del modelo**.

### Pruebas estadísticas de supuestos:

#### Test de Shapiro-Wilk para normalidad (si n <= 5000)
```{r slr-validations1-por}
if (exists("slr_model") && exists("df_por") && exists("best_predictor_por")) {  
  
  # Verificar que el modelo sea para student_por
  modelo_por <- slr_model  # Usar el modelo actual
  residuos_por <- residuals(modelo_por)
  n_obs <- length(residuos_por)
  
  cat("VALIDACIÓN DE SUPUESTOS - MODELO STUDENT_POR\n")
  cat(paste(rep("=", 45), collapse = ""), "\n")
  cat("Modelo:", deparse(formula(modelo_por)), "\n")
  cat("Observaciones:", n_obs, "\n\n")
  
  # Test de Shapiro-Wilk para normalidad (si n <= 5000)
  if (n_obs <= 5000) {
    tryCatch({
      shapiro_test <- shapiro.test(residuos_por)
      
      cat("NORMALIDAD DE RESIDUOS (Test de Shapiro-Wilk):\n")
      cat("  W =", round(shapiro_test$statistic, 4), "\n")
      cat("  p-valor =", ifelse(shapiro_test$p.value < 0.001, "< 0.001", round(shapiro_test$p.value, 4)), "\n")
      cat("  Interpretación:", ifelse(shapiro_test$p.value > 0.05, "✓ NO rechazar normalidad", "✗ RECHAZAR normalidad"), "\n")
      
      # Evaluación práctica adicional
      if (shapiro_test$p.value > 0.05) {
        cat("  Conclusión: ✓ Supuesto de normalidad CUMPLIDO\n")
      } else if (shapiro_test$statistic > 0.90) {
        cat("  Conclusión: ⚠ Ligera desviación de normalidad - ACEPTABLE para uso práctico\n")
      } else {
        cat("  Conclusión: ✗ Desviación significativa - CONSIDERAR transformaciones o modelos robustos\n")
      }
      
    }, error = function(e) {
      cat("Error en test de Shapiro-Wilk:", e$message, "\n")
    })
  } else {
    cat("NORMALIDAD DE RESIDUOS:\n")
    cat("  Muestra demasiado grande (n > 5000) para Shapiro-Wilk\n")
    cat("  Usar análisis gráfico (Q-Q plot) para evaluar normalidad\n")
  }
  
} else {
  cat("Modelo o datos no disponibles para validación de supuestos.\n")
  cat("Variables verificadas:\n")
  cat("- slr_model existe:", exists("slr_model"), "\n")
  cat("- df_por existe:", exists("df_por"), "\n") 
  cat("- best_predictor_por existe:", exists("best_predictor_por"), "\n")
}
```

#### Test de Breusch-Pagan para homocedasticidad (requiere lmtest)
```{r slr-validation2-por}
if (exists("slr_model") && exists("df_por") && exists("best_predictor_por")) {  
  
  # Verificar que el modelo sea para student_por
  modelo_por <- slr_model  # Usar el modelo actual
  
  cat("HOMOCEDASTICIDAD - MODELO STUDENT_POR\n")
  cat(paste(rep("=", 40), collapse = ""), "\n")
  ...
  if (requireNamespace("lmtest", quietly = TRUE)) {
    tryCatch({
      library(lmtest)
      bp_test <- bptest(modelo_por)
      
      cat("HOMOCEDASTICIDAD (Test de Breusch-Pagan):\n")
      cat("  BP =", round(bp_test$statistic, 4), "\n")
      cat("  gl =", bp_test$parameter, "\n")
      cat("  p-valor =", ifelse(bp_test$p.value < 0.001, "< 0.001", round(bp_test$p.value, 4)), "\n")
      cat("  Interpretación:", ifelse(bp_test$p.value > 0.05, "✓ NO rechazar homocedasticidad", "✗ RECHAZAR homocedasticidad"), "\n")
      
      # Evaluación práctica adicional
      if (bp_test$p.value > 0.05) {
        cat("  Conclusión: ✓ Supuesto de homocedasticidad CUMPLIDO\n")
      } else if (bp_test$p.value > 0.01) {
        cat("  Conclusión: ⚠ Ligera heterocedasticidad - ACEPTABLE para uso práctico\n")
      } else {
        cat("  Conclusión: ✗ Heterocedasticidad significativa - CONSIDERAR transformaciones o modelos robustos\n")
      }
      
    }, error = function(e) {
      cat("Error en test de Breusch-Pagan:", e$message, "\n")
    })
  } else {
    cat("HOMOCEDASTICIDAD:\n")
    cat("  Paquete 'lmtest' no disponible\n")
    cat("  Instalar con: install.packages('lmtest')\n")
    cat("  Usar análisis gráfico (residuos vs ajustados) para evaluar homocedasticidad\n")
  }
  
} else {
  cat("Modelo o datos no disponibles para test de homocedasticidad.\n")
  cat("Variables verificadas:\n")
  cat("- slr_model existe:", exists("slr_model"), "\n")
  cat("- df_por existe:", exists("df_por"), "\n") 
  cat("- best_predictor_por existe:", exists("best_predictor_por"), "\n")
...
```{r slr-predictions-por, results='asis'}
# Análisis de residuos y predicciones
if (exists("slr_model")) {
  ...
  # Generar predicciones con intervalos de confianza
  pred_data <- tibble(!!sym(best_predictor_por) := seq(min(df_por[[best_predictor_por]], na.rm = TRUE),
                                                    max(df_por[[best_predictor_por]], na.rm = TRUE),
                                                    length.out = 50))
  ...
  p_pred <- ggplot() +
    # Datos originales
    geom_point(data = df_por, aes_string(x = best_predictor_por, y = "G3"), 
               alpha = 0.6, size = 2, color = "#2b8cbe") +
    # Línea de regresión
    geom_line(data = pred_results, aes_string(x = best_predictor_por, y = "fit"), 
              color = "red", size = 1) +
    # Intervalos de confianza
    geom_ribbon(data = pred_results, aes_string(x = best_predictor_por, ymin = "lwr", ymax = "upr"), 
                alpha = 0.2, fill = "red") +
    labs(
      title = "Gráfico 7.3: Modelo ajustado con intervalos de confianza",
      subtitle = paste("IC del 95% para la media de G3 dado", best_predictor_por),
      x = paste(best_predictor_por, "(Variable Predictora)"),
      y = "G3 (Nota Final)"
    ) +
    theme_minimal() +
  ...
  # Tabla de predicciones ejemplo
  ejemplo_valores <- c(
    round(quantile(df_por[[best_predictor_por]], c(0.1, 0.25, 0.5, 0.75, 0.9), na.rm = TRUE))
  )
  
  pred_ejemplo <- predict(slr_model, 
                          newdata = tibble(!!sym(best_predictor_por) := ejemplo_valores), 
                          interval = "prediction", level = 0.95)
  
  ejemplo_table <- tibble(
    !!sym(paste(best_predictor_por, "(Valor)")) := ejemplo_valores,
    `G3 Predicho` = round(pred_ejemplo[,"fit"], 2),
    `Límite Inferior (95%)` = round(pred_ejemplo[,"lwr"], 2),
    `Límite Superior (95%)` = round(pred_ejemplo[,"upr"], 2)
  )
...
```{r transformations-analysis-por, results='asis'}
# PASO 1: TRANSFORMACIONES PARA MEJORAR EL MODELO
...
if (nrow(df_por) > 0 && exists("best_predictor_por")) {
  ...
  # Crear dataset con transformaciones
  df_trans <- df_por %>%
    filter(G3 > 0, !is.na(G3), !is.na(.data[[best_predictor_por]])) %>%
    mutate(
      G3_original = G3,
      G3_log = log(G3),
      G3_sqrt = sqrt(G3),
  ...
  for (i in seq_along(transformaciones)) {
    ...
    if (trans_var %in% names(df_trans)) {
      # Ajustar modelo
      formula_trans <- as.formula(paste(trans_var, "~", best_predictor_por))
      modelo_trans <- lm(formula_trans, data = df_trans)
  ...
  if (nrow(resultados_trans) > 0) {
    ...
    if (nrow(mejor_trans) > 0) {
      ...
      # Modelo original para comparación
      modelo_original <- lm(as.formula(paste("G3_original ~", best_predictor_por)), data = df_trans)
      data_orig <- augment(modelo_original)
      ...
      # Mejor transformación identificada
      if (mejor_trans$Transformacion != "G3_original") {
        modelo_mejor <- lm(as.formula(paste(mejor_trans$Transformacion, "~", best_predictor_por)), data = df_trans)
        data_mejor <- augment(modelo_mejor)
...
```{r influential-treatment-simple-por, results='asis'}
if (exists("slr_model")) {
  ...
  if (length(influential_indices) > 0) {
    ...
    # Crear tabla simple
    tabla_influyentes <- model_data[influential_indices, ] %>%
      dplyr::select(all_of(c(best_predictor_por, "G3")), .cooksd) %>%
      mutate(
        ID = influential_indices,
        .cooksd = round(.cooksd, 4)
      ) %>%
    ...
    # Crear gráfico mostrando observaciones influyentes
    p_influence <- ggplot(model_data, aes(x = .data[[best_predictor_por]], y = G3)) +
      geom_point(alpha = 0.6, color = "lightblue", size = 1) +
      geom_point(data = model_data[influential_indices, ], 
                 aes(x = .data[[best_predictor_por]], y = G3), 
                 color = "red", size = 2.5, shape = 16) +
      geom_smooth(method = "lm", se = TRUE, color = "blue", linewidth = 1) +
      labs(
        title = "Gráfico 8.2: Identificación de Observaciones Influyentes",
        subtitle = paste("Puntos rojos: observaciones con Cook's D >", round(cooksd_threshold, 6)),
        x = best_predictor_por,
        y = "G3 (Nota Final)",
        caption = paste("Total de observaciones influyentes:", length(influential_indices))
      ) +
      theme_minimal() +
...
## Necesidad de Modelos Alternativos (Student_Por)

Dado que el modelo de regresión lineal simple original **cumple mayormente los supuestos fundamentales** pero presenta algunas violaciones menores, se procede a explorar **modelos alternativos** para optimizar la validez de las inferencias estadísticas en este dataset educativo específico.

### Violaciones Identificadas en el Modelo Original

El análisis de supuestos reveló las siguientes **violaciones moderadas**:

#### **1. Normalidad de Residuos (Test de Shapiro-Wilk)**
- **Resultado:** p-valor < 0.001 (rechaza normalidad)
- **Interpretación:** Desviación significativa de la normalidad en extremos
- **Causa:** Naturaleza discreta de las calificaciones (valores enteros 0-20) y estructura bimodal característica de datos educativos

#### **2. Homocedasticidad (Test de Breusch-Pagan)**  
- **Resultado:** p-valor < 0.05 (rechaza homocedasticidad)
- **Interpretación:** Ligera heterocedasticidad detectada
- **Causa:** Variabilidad no constante en los residuos, moderada en comparación con student_mat

#### **3. Observaciones Influyentes (Distancia de Cook)**
- **Resultado:** 10-15 observaciones con Cook's D > 4/n (≈2.3% del dataset)
- **Patrón crítico:** Distribución moderada de observaciones influyentes
- **Impacto:** Influencia limitada en los coeficientes estimados

### Implicaciones de las Violaciones

Las violaciones detectadas tienen **impacto moderado** en:

- **Intervalos de confianza:** Precisión ligeramente reducida en extremos
- **Pruebas de hipótesis:** Los p-valores mantienen validez general
- **Predicciones:** Incertidumbre controlada en las estimaciones
- **Eficiencia:** El modelo OLS mantiene efectividad razonable

### Estrategias de Corrección Implementadas

Para optimizar el modelo, se evalúan **tres enfoques alternativos**:

#### **Enfoque 1: Eliminación de Observaciones Influyentes**
- **Objetivo:** Reducir el impacto limitado de observaciones atípicas
- **Método:** Remover observaciones con Cook's D > 4/n
- **Ventaja:** Mejora marginal en el cumplimiento de supuestos
- **Desventaja:** Pérdida mínima pero no justificada de información

#### **Enfoque 2: Winsorización de Datos Extremos**
- **Objetivo:** Conservar información mientras se limitan valores extremos
- **Método:** Reemplazar valores en percentiles 5% y 95% por umbrales
- **Ventaja:** Mantiene tamaño muestral completo
- **Desventaja:** Distorsión artificial innecesaria para este dataset

#### **Enfoque 3: Regresión Robusta (M-estimadores)**
- **Objetivo:** Proporcionar estimaciones resistentes a violaciones menores
- **Método:** Estimadores MM como medida preventiva
- **Ventaja:** Robustez garantizada ante violaciones detectadas
- **Desventaja:** Complejidad adicional para beneficios marginales
...
1. **Cumplimiento de supuestos estadísticos**
2. **Capacidad explicativa (R²)**
3. **Estabilidad de coeficientes**
4. **Interpretabilidad en el contexto educativo portugués**
5. **Eficiencia computacional**
6. **Justificación del incremento en complejidad**

### Metodología de Comparación

Para cada modelo alternativo se calculará:

- **Tests de normalidad** (Shapiro-Wilk para modelos lineales)
- **Tests de homocedasticidad** (Breusch-Pagan para modelos lineales)
- **Métricas de bondad de ajuste** (R², AIC, error estándar)
- **Análisis de estabilidad** de coeficientes
- **Evaluación de parsimonia** del modelo

### Justificación Metodológica

Esta aproximación comparativa garantiza:

- **Rigor estadístico:** Evaluación balanceada de alternativas
- **Eficiencia:** Selección del modelo más apropiado sin sobrecomplicar
- **Transparencia:** Documentación clara del proceso decisorio
- **Aplicabilidad:** Modelo final óptimo para predicción en contexto educativo portugués

El modelo final seleccionado será aquel que **mejor equilibre** la validez estadística con la simplicidad interpretativa, considerando que las violaciones detectadas son **manejables** en el contexto educativo.

---


```{r alternative-models-por}
# PASO 3: MODELOS ALTERNATIVOS PARA COMPARACIÓN
# =============================================
...
if (exists("slr_model") && nrow(df_por) > 0 && exists("best_predictor_por")) {
  
  # OPCIÓN A: Modelo sin observaciones influyentes
  influential_indices <- which(cooks.distance(slr_model) > 4/nrow(df_por))
  
  if (length(influential_indices) > 0 && length(influential_indices) < nrow(df_por) * 0.5) {
    df_clean <- df_por[-influential_indices, ]
    
    if (nrow(df_clean) > 50) {  # Verificar que tenemos suficientes datos
      modelo_limpio <- lm(as.formula(paste("G3 ~", best_predictor_por)), data = df_clean)
  ...
  if (requireNamespace("MASS", quietly = TRUE)) {
    library(MASS)
    tryCatch({
      modelo_robusto <- rlm(as.formula(paste("G3 ~", best_predictor_por)), data = df_por, method = "MM")

      # Calcular R² aproximado para modelo robusto - ASIGNAR A VARIABLE GLOBAL
      predicciones_rob <- predict(modelo_robusto)
      r2_robusto <<- cor(df_por$G3, predicciones_rob, use = "complete.obs")^2
  ...
  tryCatch({
    df_winsor <- df_por %>%
      mutate(
        G3_winsor = winsorize(G3),
        predictor_winsor = winsorize(.data[[best_predictor_por]])
      )
...
```{r model-selection-por, results='asis'}
# PASO 4: SELECCIÓN DEL MEJOR MODELO
# ==================================
...
## Validación final del modelos seleccionado
```{r final-validation-por, results='asis'}
# PASO 5: VALIDACIÓN FINAL DEL MODELO SELECCIONADO
# ================================================
...
```{r final-conclusions-por, results='asis'}
# PASO 6: CONCLUSIONES FINALES Y RECOMENDACIONES
# ==============================================

if (exists("modelo_final") && exists("best_predictor_por")) {
  ...
  cat("**MODELO FINAL SELECCIONADO:**", tipo_modelo, "\n")
  cat("**VARIABLE PREDICTORA:**", best_predictor_por, "\n")
  cat("**VARIABLE RESPUESTA:** G3 (Nota Final)\n\n")
  
  # Ecuación del modelo
  coefs <- coef(modelo_final)
  cat("**ECUACIÓN DEL MODELO:**\n")
  cat("G3 =", round(coefs[1], 4), "+", round(coefs[2], 4), "×", best_predictor_por, "\n\n")
  
  # Interpretación práctica
  cat("**INTERPRETACIÓN PRÁCTICA:**\n")
  cat("• Por cada punto adicional en", best_predictor_por, ", la nota final G3")
  cat(ifelse(coefs[2] > 0, " aumenta ", " disminuye "))
  cat("en promedio", round(abs(coefs[2]), 4), "puntos\n")
  ...
  if (tipo_modelo == "Robusto") {
    if (exists("datos_finales")) {
      ...
      r2_final <- cor(df_por$G3, fitted(modelo_final), use = "complete.obs")^2
    }
    cat("• El modelo explica aproximadamente", round(r2_final * 100, 2), "% de la variabilidad en G3\n")
    cat("• El modelo es robusto a observaciones atípicas\n")
  } else {
  ...
  cat("\n**FORTALEZAS DEL MODELO:**\n")
  cat("✓ Relación estadísticamente significativa entre", best_predictor_por, "y G3\n")
  cat("✓ Modelo apropiado para predicción en el contexto educativo\n")
  cat("✓ Supuestos validados o método robusto aplicado\n")
  ...
  if (exists("slr_model")) {
    ...
    tryCatch({
      # Crear gráfico de comparación
      p_final <- ggplot(df_por, aes(x = .data[[best_predictor_por]], y = G3)) +
        geom_point(alpha = 0.6, color = "lightblue", size = 1) +
        geom_smooth(method = "lm", se = FALSE, 
                   aes(linetype = "Original"), color = "blue", linewidth = 1) +
        geom_smooth(data = if(exists("datos_finales")) datos_finales else df_por,
                   method = if(tipo_modelo == "Robusto") "rlm" else "lm", 
                   se = TRUE, aes(linetype = "Final"), color = "red", linewidth = 1.2) +
        scale_linetype_manual(values = c("Original" = "dashed", "Final" = "solid")) +
        labs(
